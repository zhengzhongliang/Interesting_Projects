...
def rl():
    ...
    for episode in range(MAX_EPISODES):
        ...
        while not is_terminated:
            ...
            if S_ != 'terminal':
                q_target = R + GAMMA * q_table.iloc[S_, :].max()+MAX_H_REWARD/(N_STATES-S_)   # next state is not terminal
            else:
...
if __name__ == "__main__":
    ...
    plt.figure()
    plt.plot(STEP_LIST)
    plt.title('step-epoch (heuristic - goal distance)')
    plt.xlabel('epochs')
    plt.ylabel('steps when finishing')
    plt.ylim((0, 40))
    plt.show()
