...
class DNC:
    def __init__(self, input_size, output_size, seq_len, num_words=256, word_size=64, num_heads=4):
        ...
        self.i_data = tf.placeholder(tf.float32, [seq_len, self.input_size], name='input_node')
        self.o_data = tf.placeholder(tf.float32, [seq_len, self.output_size], name='output_node')
...
def main(argv=None):
    ...
    num_seq = 10
    seq_len = 28    #=time step
    seq_width = 28   #=dimension of each time step
    epochs = 20
    ...
    with graph.as_default():
        ...
        # if we increase the num_words and word_size, the system will become unstable
        ...
        loss = tf.reduce_mean(tf.losses.mean_squared_error(labels=dnc.o_data, predictions=output))
        ...
        with tf.Session(graph=graph) as sess:
            ...
            for i in np.arange(epochs):
                ...
                for j in np.arange(len(x_train)):
                    final_i_data = x_train[j,:,:]
                    final_o_data = x_train[j,:,:]
                    ...
                    epoch_loss+=l
                    if j%6000==0:
                        print("average loss:",epoch_loss/(j+1))
                        test_index = int(4900*np.random.rand())
                        test_image = np.zeros((28,28))
                        plt.figure()
                        for k in np.arange(10):
                            test_image, = sess.run([output], feed_dict={dnc.i_data:x_test[test_index+k,:,:], dnc.o_data:x_test[test_index+k,:,:]})
                            plt.subplot(2,5,k+1)
                            plt.imshow(test_image)
                        plt.show()
                print('epoch:',i+1, '   loss:',epoch_loss/60000)
            #print results
            # print(final_i_data)
            # print(final_o_data)
            # print(predictions)
            save_path_ = "DNC_trained_model/DNC_1"
            if not os.path.exists(save_path_):
                os.makedirs(save_path_)
            save_path = saver.save(sess, save_path_, global_step=epochs)

    if flag_testing:
        with tf.Session(graph=graph) as sess:
            saver.restore(sess, tf.train.latest_checkpoint('DNC_trained_model'))
