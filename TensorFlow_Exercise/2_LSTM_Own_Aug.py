...
class LSTM():
  ...
  def __init__(self, time_step, input_size, hidden_size, output_size):
    ...
    self.b_g = tf.get_variable(name='b_g',shape=[hidden_size],dtype=tf.float32)
    self.b_i = tf.get_variable(name='b_i',shape=[hidden_size],dtype=tf.float32)
    self.b_f = tf.get_variable(name='b_f',shape=[hidden_size],dtype=tf.float32)
    self.b_o = tf.get_variable(name='b_o',shape=[hidden_size],dtype=tf.float32)
    self.b_v = tf.get_variable(name='b_v',shape=[output_size],dtype=tf.float32)
    ...
    for i in np.arange(time_step):
      if(i==0):
        ...
        g = tf.tanh(tf.matmul(x_current,self.U_g)+self.b_g)
        i = tf.sigmoid(tf.matmul(x_current,self.U_i)+self.b_i)
        f = tf.sigmoid(tf.matmul(x_current,self.U_f)+self.b_f)
        o = tf.sigmoid(tf.matmul(x_current,self.U_o)+self.b_o)
        c_current = tf.multiply(g,i)
        h_current = tf.multiply(o,tf.tanh(c_current))
        y_current = tf.matmul(h_current, self.V)+self.b_v    # y_current(1,2)
        ...
        x_current = tf.reshape(self.x[i,:],shape=[1, input_size])
        g = tf.tanh(tf.add(tf.matmul(x_current,self.U_g), tf.matmul(h_prev,self.W_g))+self.b_g)
        i = tf.sigmoid(tf.add(tf.matmul(x_current,self.U_i), tf.matmul(h_prev,self.W_i))+self.b_i)
        f = tf.sigmoid(tf.add(tf.matmul(x_current,self.U_f), tf.matmul(h_prev,self.W_f))+self.b_f)
        o = tf.sigmoid(tf.add(tf.matmul(x_current,self.U_o), tf.matmul(h_prev,self.W_o))+self.b_o)
    ...
    self.tvars=[self.V, self.W_g, self.W_i,self.W_f,self.W_o,self.U_g,self.U_i,self.U_f,self.U_o, self.b_g, self.b_i, self.b_f, self.b_o, self.b_v]
...
rnn = LSTM(time_step=28, input_size=28, hidden_size=32, output_size=10)
