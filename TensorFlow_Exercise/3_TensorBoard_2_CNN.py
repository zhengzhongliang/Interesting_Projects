...
## CNN Prototype: https://github.com/aymericdamien/TensorFlow-Examples/, Author:Aymeric Damien
...
def main(is_train, is_test, save_path_, cal_CENT):
    ...
    writer=tf.summary.FileWriter('event2')
    writer.add_graph(g)
    if is_train:
...
is_train=True   # start training the CNN by setting this to True. If the script is run for the first time, set this to True.
...
cal_CENT=False    # whether to save the layer outputs.
